...
@router.get("/by-name/{advisor_name}", response_model=AdvisorResponse)
def get_advisor_by_name(advisor_name: str, db: Session = Depends(get_db)):
    advisor = (
        db.query(Advisor)
        .filter(Advisor.name.ilike(advisor_name))
        .first()
    )
    if not advisor:
        raise HTTPException(status_code=404, detail="Adviseur niet gevonden")
...
@router.patch("/{advisor_id}", response_model=AdvisorResponse)
def update_advisor(
    advisor_id: int, advisor: AdvisorUpdate, db: Session = Depends(get_db)
):
    db_advisor = db.query(Advisor).filter(Advisor.id == advisor_id).first()
    if not db_advisor:
        raise HTTPException(status_code=404, detail="Adviseur niet gevonden")
